...
import { ApiError, createSubmission, getJob, retryJob } from "./lib/api";
...
const fileError = (file: File | null) => !file ? "Choose a PDF file." : (!file.name.toLowerCase().endsWith(".pdf") || file.type !== "application/pdf") ? "Choose a PDF file." : undefined;
...
export function FounderPortal() {
  ...
  const chooseSingle = (setter: (file: File | null) => void, name: "pitch_deck" | "cv") => (event: ChangeEvent<HTMLInputElement>) => { setter(event.target.files?.[0] ?? null); setErrors((current) => ({ ...current, [name]: undefined })); };
  const addSupporting = (event: ChangeEvent<HTMLInputElement>) => { const files = Array.from(event.target.files ?? []); setSupporting((current) => [...current, ...files].slice(0, maxSupporting)); setErrors((current) => ({ ...current, supporting_documents: undefined })); event.target.value = ""; };
  const replaceSupporting = (index: number) => (event: ChangeEvent<HTMLInputElement>) => { const replacement = event.target.files?.[0]; if (replacement) setSupporting((current) => current.map((file, position) => position === index ? replacement : file)); };
...
function Nav() { return <header className="navbar"><a href="/" className="brand" aria-label="Sentient Ventures submission home"><img src={logo} alt="Sentient Ventures" /><span>Sentient<br />Ventures</span></a><p>Founder submission portal</p></header>; }
