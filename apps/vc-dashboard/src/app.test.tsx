...
describe("DashboardApp", () => {
  ...
  it("renders API criterion content and the unavailable score state", async () => { render(<DashboardApp />); expect(await screen.findByRole("heading", { name: "Aether Robotics" })).toBeInTheDocument(); expect(screen.getByText("What is the company idea?")).toBeInTheDocument(); fireEvent.click(screen.getByRole("link", { name: /market/i })); expect(await screen.findByRole("heading", { name: "Market evaluation" })).toBeInTheDocument(); expect(screen.getByText("Does the company fit the VC portfolio?")).toBeInTheDocument(); expect(screen.getByLabelText("Score: unavailable")).toBeInTheDocument(); expect(screen.getByLabelText("Criterion score: unavailable")).toBeInTheDocument(); });
  it("shows deterministic loaded investment terms in the dashboard metadata", async () => { api.getCompany.mockResolvedValue({ ...evaluation(aether), investment: { amount: 100000, currency: "EUR", equityPercentage: 10, preMoneyValuation: null, postMoneyValuation: null, impliedValuation: null, useOfFunds: [] } }); render(<DashboardApp />); expect(await screen.findByLabelText("Investment terms: €100k for 10%")).toBeInTheDocument(); });
