...
describe("DashboardApp", () => {
  ...
  it("keeps criteria closed while preserving their title and score, then reveals clean content through the native summary", async () => { render(<DashboardApp />); expect(await screen.findByRole("heading", { name: "Aether Robotics" })).toBeInTheDocument(); const title = screen.getByRole("heading", { name: "What is the company idea?" }); const details = title.closest("details"); const summary = title.closest("summary"); expect(details).not.toHaveAttribute("open"); expect(within(details!).getByLabelText("Score: unavailable")).toBeInTheDocument(); expect(within(details!).getByLabelText("Criterion score: unavailable")).toBeInTheDocument(); expect(summary).not.toBeNull(); fireEvent.click(summary!); expect(details).toHaveAttribute("open"); expect(within(details!).getByText("Aether Robotics assessment.")).toBeInTheDocument(); expect(within(details!).getByText("Useful submitted evidence.")).toBeInTheDocument(); expect(within(details!).queryByText("Hidden technical source reference.")).not.toBeInTheDocument(); expect(within(details!).queryByRole("heading", { name: "Source references" })).not.toBeInTheDocument(); fireEvent.click(screen.getByRole("link", { name: /market/i })); expect(await screen.findByRole("heading", { name: "Market evaluation" })).toBeInTheDocument(); const portfolioDetails = screen.getByText("Does the company fit the VC portfolio?").closest("details"); expect(portfolioDetails).not.toHaveAttribute("open"); expect(within(portfolioDetails!).getByLabelText("Criterion score: unavailable")).toBeInTheDocument(); });
  it("shows the fixed €100k Aether terms in the dashboard metadata", async () => { api.getCompany.mockResolvedValue({ ...evaluation(aether), investment: { amount: 100000, currency: "EUR", equityPercentage: 1, preMoneyValuation: 9900000, postMoneyValuation: 10000000, impliedValuation: 10000000, useOfFunds: [] } }); render(<DashboardApp />); expect(await screen.findByLabelText("Investment terms: €100k for 1%")).toBeInTheDocument(); });
