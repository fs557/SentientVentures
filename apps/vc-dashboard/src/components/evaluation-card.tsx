...
function EvidenceNotes({ entries }: { entries: EvaluationItem["evidence"] }) {
  return <section className="evidence"><h4>Evidence notes</h4><ul>{entries.map((entry, index) => <li key={`${entry.documentId}-${index}`}>{entry.text}</li>)}</ul></section>;
}
export function EvaluationCard({ item }: { item: EvaluationItem }) {
  return <details className="evaluation-card" aria-labelledby={`${item.id}-title`}>
    ...
    <div className="evaluation-card__content">
      ...
      <EvidenceNotes entries={item.evidence} />
