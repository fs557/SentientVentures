...
function SourceExcerpts({ entries, assessment }: { entries: EvaluationItem["evidence"]; assessment: string }) {
  const normalizedAssessment = assessment.trim().replace(/\s+/g, " ").toLowerCase();
  const excerpts = entries.filter((entry) => entry.kind === "fact" && entry.text.trim().replace(/\s+/g, " ").toLowerCase() !== normalizedAssessment);
  if (excerpts.length === 0) return null;
  return <section className="evidence"><h4>Source excerpts</h4><ul>{excerpts.map((entry, index) => <li key={`${entry.documentId}-${index}`}>{displayText(entry.text)}</li>)}</ul></section>;
}
export function EvaluationCard({ item, variant = "criterion" }: { item: EvaluationItem; variant?: "criterion" | "fact" }) {
  ...
  return <details className={`evaluation-card evaluation-card--${variant}`} aria-labelledby={`${item.id}-title`}>
    ...
    <div className="evaluation-card__content">
      ...
      {!isFact && <SourceExcerpts entries={item.evidence} assessment={item.assessment} />}
