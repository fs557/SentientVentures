...
export function HomeOverview({ evaluation }: { evaluation: CompanyEvaluation }) {
  ...
  return <>
    ...
    <section className="signals" aria-label="Investment signals"><article><h2>Investment arguments</h2><ul>{argumentsFor.length ? argumentsFor.map((entry, index) => <li key={`${entry.id}-${index}`}>{entry.text}</li>) : <li>Not provided</li>}</ul></article><article><h2>Key risks</h2><ul>{risks.length ? risks.map((entry, index) => <li key={`${entry.id}-${index}`}>{entry.text}</li>) : <li>Not provided</li>}</ul></article><article><h2>Diligence priorities</h2><p>{diligencePriorities}</p></article></section>
